...
```{r}
Baseline <- c(base_mean,base_mean)
In_Session <- c(beh_C,beh_NC)
Prior_Session <- c(Prior_C, Prior_NC)
Full_Session <- c(F_C, F_NC)
df<- data.frame(Baseline,In_Session, Prior_Session,Full_Session)
rownames(df)<- c("with click", "no click")
df
```
